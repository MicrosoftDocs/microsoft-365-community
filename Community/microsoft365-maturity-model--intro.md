...
Our goal is to apply the same core Competencies that were the core of the original SPMM, updated and extended to reflect the current business and technical environment. Because Microsoft 365 is a much deeper and wider toolkit, the project is creating a guidance document for each Competency, in a consistent format. Although these are clearly linked to M365, we have deliberately avoided detailing particular features and functions, focusing on the business needs and processes in the Competency documents. We will continue to expand the document set to drill into the technologies; a ‘How to’ for achieving different levels with the tools M365 provides.  
