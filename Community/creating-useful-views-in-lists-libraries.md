...
TIP: Instead of going into the View properties to modify an existing view, you can make the changes you want by applying sorting and filtering directly on the list or library and then saving the view. This technique allows you to apply a filter on a multi-select choice column, which is otherwise not possible. (The Edit View page in Settings does not support defining this type of filter.) 
